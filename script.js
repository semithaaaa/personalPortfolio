document.addEventListener("DOMContentLoaded", () => {
  ...
  function initParticleBackground() {
    ...
    class Particle {
      ...
      draw() {
        if (isLightTheme()) {
          ctx.fillStyle = `rgba(0, ${Math.floor(this.g * 0.6)}, ${Math.floor(this.b * 0.65)}, ${this.alpha + 0.15})`
        } else {
          ctx.fillStyle = `rgba(0, ${this.g}, ${this.b}, ${this.alpha})`
        }
        ctx.beginPath()
      ...
      for (let a = 0; a < particles.length; a++) {
        for (let b = a; b < particles.length; b++) {
          ...
          if (distance < maxDistance) {
            ...
            if (light) {
              ctx.strokeStyle = `rgba(0, 120, 130, ${opacity * 0.15})`
            } else {
